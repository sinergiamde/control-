...
const History = () => {
  ...
  const fetchAnnualSummary = async () => {
    // An annual summary belongs to exactly one client — without client_id in the filter, two
    // different clients' 2025 summaries would show/overwrite each other under the same "year 2025"
    // row. Requires a client to be picked first (same rule the Dashboard upload already enforces).
    if (!user || !/^\d{4}$/.test(range) || !clientFilter) { setAnnualSummary(null); return; }
    const { data } = await supabase
      .from("annual_summaries")
      .select("generated_at, net_income")
      .eq("year", Number(range))
      .eq("client_id", clientFilter.id)
  ...
  const handleGenerateAnnual = async () => {
    ...
    if (!clientFilter) {
      toast({ title: "⚠️", description: tr(STR.clientRequiredForAnnual, isEnglish), variant: "destructive" });
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/annual-summary`, {
        ...
        body: JSON.stringify({ year: Number(range), client_id: clientFilter.id }),
      });
  ...
  const duplicatePeriodIds = useMemo(() => {
    // Keyed by client + period, never period alone — history is now shared across every client,
    // so two unrelated companies both having a "2025-01" statement is normal, not a duplicate.
    // Without the client_id in the key this fired a false "duplicate period" warning on every
    // upload once a second client existed in the same table.
    const groups: Record<string, AnalysisRow[]> = {};
    rows.forEach((r) => {
      const period = normalizePeriod(r.period || "");
      if (!period) return;
      const key = `${r.client_id || "none"}::${period}`;
  ...
  const handleDownload = async (format: "excel" | "pdf") => {
    ...
    try {
      const isEnglish = lang === "en";
      // The report's header must show the CLIENT's name (e.g. "RAMAS DRYWALL INC"), never the
      // logged-in staff account's own profile name — that was the bug behind "el resumen anual me
      // sale a nombre de TAXFOURYOU INC" when it should say the actual client being reported on.
      const consolidated = buildConsolidatedReport(
        filtered.map((r) => r.full_analysis),
        clientFilter?.name || tr(STR.clientFilterAllLabel, isEnglish),
        isEnglish
      );
      if (format === "excel") {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {/^\d{4}$/.test(range) && !clientFilter && (
              <div className="mb-4 flex items-center gap-2 text-xs rounded-lg border border-border bg-muted/40 px-3 py-2 text-muted-foreground">
                <CalendarCheck className="h-4 w-4 shrink-0" />
                {tr(STR.clientRequiredForAnnual, isEnglish)}
              </div>
            )}
            {/^\d{4}$/.test(range) && clientFilter && (
              annualSummary ? (
